 ...
 * 
 * $Id: ns_suspend.c 1265 2007-08-26 15:33:06Z geniusdex $
 *
 */
/*************************************************************************/
...
int AnopeInit(int argc, char **argv)
{
    ...
    moduleAddVersion("$Id: ns_suspend.c 1265 2007-08-26 15:33:06Z geniusdex $");
    moduleSetType(CORE);
...
int do_suspend(User * u)
{
    ...
    if (na) {
        ...
        if (WallForbid)
            xanadu_cmd_global(s_NickServ, "\2%s\2 used SUSPEND on \2%s\2",
                             u->nick, nick);
...
int do_unsuspend(User * u)
{
    ...
    if (na) {
        ...
        if (WallForbid)
            xanadu_cmd_global(s_NickServ, "\2%s\2 used UNSUSPEND on \2%s\2",
                             u->nick, nick);
