 ...
 * 
 * $Id: ns_drop.c 1265 2007-08-26 15:33:06Z geniusdex $
 *
 */
/*************************************************************************/
...
int AnopeInit(int argc, char **argv)
{
    ...
    moduleAddVersion("$Id: ns_drop.c 1265 2007-08-26 15:33:06Z geniusdex $");
    moduleSetType(CORE);
...
int do_drop(User * u)
{
    ...
    if (is_mine && !nick_identified(u)) {
        ...
        if (!is_mine) {
            if (WallDrop)
                xanadu_cmd_global(s_NickServ, "\2%s\2 used DROP on \2%s\2",
                                 u->nick, nick);
            notice_lang(s_NickServ, u, NICK_X_DROPPED, nick);
        } else {
            if (nick)
