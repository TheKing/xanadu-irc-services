 ...
 * 
 * $Id: cs_topic.c 1322 2007-12-28 19:12:02Z geniusdex $
 *
 */
/*************************************************************************/
...
int AnopeInit(int argc, char **argv)
{
    ...
    moduleAddVersion("$Id: cs_topic.c 1322 2007-12-28 19:12:02Z geniusdex $");
    moduleSetType(CORE);
...
int do_cs_topic(User * u)
{
    ...
    if (!chan) {
        ...
        xanadu_cmd_topic(whosends(ci), c->name, u->nick, topic ? topic : "",
                        c->topic_time);
        if (ircd->join2set) {
            if (whosends(ci) == s_ChanServ) {
