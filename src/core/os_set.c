 ...
 * 
 * $Id: os_set.c 1322 2007-12-28 19:12:02Z geniusdex $
 *
 */
/*************************************************************************/
...
int AnopeInit(int argc, char **argv)
{
    ...
    moduleAddVersion("$Id: os_set.c 1322 2007-12-28 19:12:02Z geniusdex $");
    moduleSetType(CORE);
    ...
    if (!option) {
        ...
        if (LogChannel && (stricmp(setting, "on") == 0)) {
            if (ircd->join2msg) {
                ...
                xanadu_cmd_join(s_GlobalNoticer, LogChannel, c ? c->creation_time : time(NULL));
            }
            logchan = 1;
            ...
            if (ircd->join2msg) {
                xanadu_cmd_part(s_GlobalNoticer, LogChannel, NULL);
            }
            logchan = 0;
