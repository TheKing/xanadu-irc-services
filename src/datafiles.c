 ...
 * 
 * $Id: datafiles.c 1265 2007-08-26 15:33:06Z geniusdex $ 
 *
 */
    ...
    if (!fp) {
        ...
        if (errno != ENOENT)
            log_perror("Can not read %s database %s", service,
                       f->filename);
        if (time(NULL) - lastwarn > WarningTimeout) {
            xanadu_cmd_global(NULL, "Write error on %s: %s", f->filename,
                             strerror(errno));
            lastwarn = time(NULL);
        }
#endif
                             ...
                             uint32 version)
{
    ...
    *f->backupname = 0;
    ...
    if (rename(filename, f->backupname) < 0 && errno != ENOENT) {
        ...
        if (!walloped) {
            walloped++;
            xanadu_cmd_global(NULL, "Can not back up %s database %s",
                             service, filename);
        }
#ifdef _WIN32
        if (debug) {
    ...
    if (!f->fp || !write_file_version(f, version)) {
        ...
        if (!walloped) {
            walloped++;
            xanadu_cmd_global(NULL, "Can't write to %s database %s",
                             service, filename);
        }
        errno = errno_save;
...
static void rename_database(char *name, char *ext)
{
    ...
    if (rename(name, destpath) != 0) {
        ...
        xanadu_cmd_global(s_OperServ, "WARNING! Backup of %s failed.",
                         name);
    }
}
